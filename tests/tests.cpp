...
TEST_CASE( "Wrapper member function signature helper static assertions", "[signaturehelper]" )
{
...
TEST_CASE( "Member function signature helper static assertions", "[signaturehelper]" )
{
...
TEST_CASE( "Wrapper copy moves", "[wrapper]" )
{
  ...
  REQUIRE(wrapper_r_value_ref_arg.call<CopyCounter>(std::move(counter_for_r_values_1)).first == 0);
  auto counter_for_r_values_2 = CopyMoveCounter{};
  REQUIRE(wrapper_r_value_ref_arg.call<CopyCounter>(std::move(counter_for_r_values_2)).second == 2);
}
