...
TEST_CASE("Member function signature helper static assertions",
          "[signaturehelper]") {
  using TestConst =
      gte::detail::MemberFunctionSignatureHelper<decltype(&Tester::forty_two)>;
  ...
  using TestNonConst = gte::detail::MemberFunctionSignatureHelper<
      decltype(&Tester::set_forty_two)>;
...
TEST_CASE("Add arguments to signature helper", "[signaturehelper]") {
  using TestWrapperMemFun =
      gte::detail::SignatureWithExtraArgs<int(double, int), const std::any &,
                                          const std::any &>;
...
TEST_CASE("Pairs to tuples", "[pairstotuples]") {
  ...
  static_assert(std::is_same_v<typename Tuples::FirstTuple, std::tuple<int> >);
  static_assert(
