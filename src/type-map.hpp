...
constexpr auto
get_index (std::index_sequence<Indices...>) -> std::size_t
{
...
template <typename T, typename Tuple, std::size_t... Indices>
constexpr auto
has_type (std::index_sequence<Indices...>) -> bool
{
...
template <typename T, typename Tuple, std::size_t... Indices>
constexpr auto
type_count (std::index_sequence<Indices...>) -> std::size_t
{
...
template <typename T, typename Tuple, std::size_t... Indices>
constexpr auto
type_is_unique (std::index_sequence<Indices...> indices) -> std::size_t
{
...
constexpr auto
all_types_unique (std::index_sequence<Indices...> indices) -> bool
{
  return (type_is_unique<std::tuple_element_t<Indices, Tuple>, Tuple> (indices)
          && ...);
}
}

template <typename... KeyValuePairs> struct TypeMap
{
  ...
  using KeyTuple =
      typename detail::PairsToTuples<KeyValuePairs...>::FirstTuple;
  using ValueTuple =
